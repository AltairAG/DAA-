...
def grafoGeografico(n, r, dirigido=False):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if distancia <= r:
                # Crear solo una arista si no es dirigido
                grafo.crear_aristas(grafo.lista_nodos[i].id, grafo.lista_nodos[j].id)
                if dirigido:
                    # Si es dirigido, crea también la arista en sentido contrario
                    grafo.crear_aristas(grafo.lista_nodos[j].id, grafo.lista_nodos[i].id)
